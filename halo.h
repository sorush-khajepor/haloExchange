...
struct Box{
    ...
    Box(const std::array<int,2>& low_,const std::array<int,2>& high_):
        low(low_), high(high_){}
    Box(int ix0, int ix1, int iy0, int iy1):
        low({ix0,iy0}), high({ix1, iy1}){}
...
struct Block{
    ...
    void communicate(Axis axis){
        const auto& [dir1, dir2] = GetDirsOfAxis[axis];
        std::vector<MPI_Request> requests;
        for (auto& neighbour: neighbours){
            const auto& [cpuId, dir]= neighbour;
            if (dir==dir1 || dir==dir2)
                requests.push_back(communicate(dir, cpuId));
        }
        for (auto& r:requests){
            MPI_Status s;
            MPI_Wait(&r, &s);
    ...
    MPI_Request communicate(Dir dir, int dest){
